...
import booleanFakerFactory from "./booleanFakerFactory";
...
const BooleanFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = booleanFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("boolean").create(elem));
...
export default connect(mapState, mapDispatch)(BooleanFakerDialogComponent);
