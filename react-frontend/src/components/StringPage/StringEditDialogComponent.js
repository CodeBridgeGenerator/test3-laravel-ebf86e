...
const StringCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        const result = await client.service("string").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info string updated successfully" });
    ...
    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="string-edit-dialog-component">
                <div>
...
export default connect(mapState, mapDispatch)(StringCreateDialogComponent);
