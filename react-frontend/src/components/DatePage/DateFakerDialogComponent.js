...
import dateFakerFactory from "./dateFakerFactory";
...
const DateFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dateFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("date").create(elem));
...
export default connect(mapState, mapDispatch)(DateFakerDialogComponent);
