...
import objectFakerFactory from "./objectFakerFactory";
...
const ObjectFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = objectFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("object").create(elem));
...
export default connect(mapState, mapDispatch)(ObjectFakerDialogComponent);
