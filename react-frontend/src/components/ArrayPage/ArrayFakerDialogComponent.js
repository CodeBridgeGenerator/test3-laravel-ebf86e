...
import arrayFakerFactory from "./arrayFakerFactory";
...
const ArrayFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = arrayFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("array").create(elem));
...
export default connect(mapState, mapDispatch)(ArrayFakerDialogComponent);
