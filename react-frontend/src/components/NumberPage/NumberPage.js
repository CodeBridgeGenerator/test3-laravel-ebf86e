...
const NumberPage = (props) => {
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("number").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    return (
        <div className="col-12 flex flex-column align-items-center">
            ...
            <NumberEditDialogComponent entity={data[selectedEntityIndex]} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
