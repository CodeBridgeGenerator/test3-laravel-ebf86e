...
import numberFakerFactory from "./numberFakerFactory";
...
const NumberFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = numberFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("number").create(elem));
...
export default connect(mapState, mapDispatch)(NumberFakerDialogComponent);
