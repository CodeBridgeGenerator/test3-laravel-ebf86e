...
import relationFakerFactory from "./relationFakerFactory";
...
const RelationFakerDialogComponent = (props) => {
    ...
    const [loading, setLoading] = useState(false);
                ...
                const [profileEmailIds, setprofileEmailIds] = useState();
...
client
            ...
            .then((data) => {
              setprofileEmailIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = relationFakerFactory(fakerCount,profileIds,profileEmailIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("relation").create(elem));
...
export default connect(mapState, mapDispatch)(RelationFakerDialogComponent);
